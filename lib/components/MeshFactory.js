...
export class MeshFactory {
  ...
  /**
   * A 3D semi-circular region in the horizontal plane with an expanding/contracting ring.
   * Used to provide information about reach distance without showing exact position.
   * @returns {Mesh}
   */
  ...
  static noFeedbackZone3D({ near = 0.01, far = 0.2, opacity = 0.05 }) {
    const zone = new Mesh(
      new RingGeometry(near, far),
      new MeshBasicMaterial({
        transparent: true,
        opacity: opacity,
        color: '#FF8C00',
    ...
    zone.far = far;
    // Rotate back so local XY lies in world XZ
    // zone.rotateZ(Math.PI / 2);
    // zone.rotateX(Math.PI / 2);
    //  zone.rotateZ((-0.5 * Math.PI) / 2);
    zone.renderOrder = Infinity;
