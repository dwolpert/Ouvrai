...
export function generateDemoReaches_specific({
  object = required('object'),
  Angle = required('maxAngle'),
  distance = required('distance'),
  duration = required('duration'),
  initialDelay = 800,
  repeatDelay,
  ...
  // We want to translate distance in a random direction within maxAngle
  let disp = new Vector3(Math.tan(Angle) * distance, 0, -1 * distance);
  let end = object.position.clone().add(disp);
  ...
  if (recursive) {
    tween.onComplete(
      () =>
        (object.demoTween = generateDemoReaches_specific({
          object: object,
          Angle: Angle,
          distance: distance,
          duration: duration,
