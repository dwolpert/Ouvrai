...
export class Experiment {
  ...
  constructor(cfg) {
    ...
    // this.cfg.backgroundColor ??= '#FFE4B5';
    this.cfg.backgroundColor ??= '#FFE4B5';
  ...
  createTrialSequence(blocks, append = false) {
    // Input validation: Check if 'blocks' is an array of objects
    if (!Array.isArray(blocks) || !blocks.every((x) => typeof x === 'object')) {
      throw new Error(`Argument 'blocks' must be an array of Block.`);
    }
    // If 'append' is false, clear existing trials
    if (!append) this.trials = [];

    // Calculate the cycleNumber based on the last trial's cycle or 0 if no trials exist
    let cycleNumber = this.trials.slice(-1)[0]?.cycle || 0;

    // Iterate through the 'blocks' array
    for (let [blockNumber, bk] of blocks.entries()) {
      // For each block, map over its trials
      bk.trials.map((t) => {
        // Assign 'blockNumber' to each trial
        t.blockNumber = blockNumber;
        // Calculate 'cycle' for each trial based on 'repetition' and 'cycleNumber'

        t.cycle = t.block.repetition + cycleNumber;
      });
      // Push the trials from the current block to the 'this.trials' array
      this.trials.push(...bk.trials);
      // Update 'cycleNumber' for the next block
      cycleNumber = this.trials.slice(-1)[0]?.cycle + 1;
    }

    // Update the total number of trials
    this.numTrials = this.trials.length;
    // Update progress (context of 'progress' variable is not shown)
    this.progress.update(this.trialNumber, this.numTrials);
    // Log the created trial sequence
